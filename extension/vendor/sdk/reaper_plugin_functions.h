...
//   __langpack_filename can query langpack filename. If empty or <> then no langpack loaded; if relative pathname, then langpack is not yet loaded.
//   __reascript_runcnt: increments with each deferred timer slice.
//   __linux_rt_prio: realtime thread priority base level (linux only).
...
// gets ini configuration variable value as string. For most keys, this will get the in-memory copy of it, but for others it will actually read the .ini file directly.
...
// VKB_NOTECENTER : virtual MIDI keyboard center note
// VKB_LASTVEL : virtual MIDI keyboard last velocity
// VKB_CHANNEL : virtual MIDI keyboard channel
...
#if defined(REAPERAPI_WANT_GetSetTempoTimeSigMarkerBasis) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
...
// Gets or sets the beat basis of a tempo/time signature marker. Supported values are 0.5 (eighth note), 0.75 (dotted eighth), 1.0 (quarter), 1.5 (dotted quarter), 2.0 (half), 3.0 (dotted half).
  ...
  double (*REAPERAPI_FUNCNAME(GetSetTempoTimeSigMarkerBasis))(ReaProject* project, int point_index, double beatbase, bool is_set);
#endif
...
// pixels_per_timebase_unit: returns 1024*pixels per timebase unit (second or QN)
// timebase_unit: returns 0 if timebase unit is seconds, 1 if QN 
...
// pixels_per_timebase_unit: set to 1024*pixels per timebase unit (second or QN)
...
// sets ini configuration variable value as string (converted to the appropriate type), in memory. Return value is 0 if failed, 1 if global preference, 2 if project setting. If persist is set to the correct type (1=global preference, 2=project default), then value will be written to reaper.ini. Note that some ini keys can only be written with persist=3, these will be updated in the .ini file but may not take effect immediately (equivalent of WritePrivateProfileString). Note that when using persist=3, you may delete the key by writing the special value '=delete='
...
// param.X.default_value : returns normalized default value of parameter, if available
// param_hovered : returns parameter mouse is above, or -1 if no parameter, if supported by plug-in (currently only some CLAP plug-ins support this). If not supported, call will fail.
...
#if defined(REAPERAPI_WANT_Undo_GetCurEntry) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
// Undo_GetCurEntry
// Gets current undo entry index

  int (*REAPERAPI_FUNCNAME(Undo_GetCurEntry))(ReaProject* proj);
#endif

#if defined(REAPERAPI_WANT_Undo_GetEntryDesc) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
// Undo_GetEntryDesc
// Gets description of undo entry.

  const char* (*REAPERAPI_FUNCNAME(Undo_GetEntryDesc))(ReaProject* proj, int index);
#endif

#if defined(REAPERAPI_WANT_Undo_GetEntryTime) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
// Undo_GetEntryTime
// Gets timestamp (since Jan 1 1970) of undo entry.

  double (*REAPERAPI_FUNCNAME(Undo_GetEntryTime))(ReaProject* proj, int index);
#endif

#if defined(REAPERAPI_WANT_Undo_GetNumEntries) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
// Undo_GetNumEntries
// Gets number of undo entries

  int (*REAPERAPI_FUNCNAME(Undo_GetNumEntries))(ReaProject* proj);
#endif

#if defined(REAPERAPI_WANT_Undo_IsEntryAltTree) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
// Undo_IsEntryAltTree
// Returns number of alternate redo paths available (0 if none, 1 if a single alternate exists, etc).

  int (*REAPERAPI_FUNCNAME(Undo_IsEntryAltTree))(ReaProject* proj, int index);
#endif
...
#if defined(REAPERAPI_WANT_Undo_SetCurPos) || !defined(REAPERAPI_MINIMAL)
REAPERAPI_DEF //==============================================
// Undo_SetCurPos
// Loads an undo entry. If loadAltTree is set to a positive value, the next alternate tree redo branch is loaded. If loadAltTree is set to a negative value, the previous alternate tree redo branch is loaded.

  void (*REAPERAPI_FUNCNAME(Undo_SetCurPos))(ReaProject* proj, int index, int loadAltTreeOptional);
#endif
...
// WDL_VirtualWnd_ScaledBlitBG
  ...
  int REAPERAPI_LoadAPI(void *(*getAPI)(const char *))
  {
    static const struct { void **dest; const char *name; } table[]={
      ...
      #endif
      #if defined(REAPERAPI_WANT_GetSetTempoTimeSigMarkerBasis) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(GetSetTempoTimeSigMarkerBasis),"GetSetTempoTimeSigMarkerBasis"},
      #endif
      #if defined(REAPERAPI_WANT_GetSetTempoTimeSigMarkerFlag) || !defined(REAPERAPI_MINIMAL)
      ...
      #endif
      #if defined(REAPERAPI_WANT_Undo_GetCurEntry) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(Undo_GetCurEntry),"Undo_GetCurEntry"},
      #endif
      #if defined(REAPERAPI_WANT_Undo_GetEntryDesc) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(Undo_GetEntryDesc),"Undo_GetEntryDesc"},
      #endif
      #if defined(REAPERAPI_WANT_Undo_GetEntryTime) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(Undo_GetEntryTime),"Undo_GetEntryTime"},
      #endif
      #if defined(REAPERAPI_WANT_Undo_GetNumEntries) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(Undo_GetNumEntries),"Undo_GetNumEntries"},
      #endif
      #if defined(REAPERAPI_WANT_Undo_IsEntryAltTree) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(Undo_IsEntryAltTree),"Undo_IsEntryAltTree"},
      #endif
      #if defined(REAPERAPI_WANT_Undo_OnStateChange) || !defined(REAPERAPI_MINIMAL)
      ...
      #endif
      #if defined(REAPERAPI_WANT_Undo_SetCurPos) || !defined(REAPERAPI_MINIMAL)
        {(void**)&REAPERAPI_FUNCNAME(Undo_SetCurPos),"Undo_SetCurPos"},
      #endif
      #if defined(REAPERAPI_WANT_update_disk_counters) || !defined(REAPERAPI_MINIMAL)
